...
// دالة لتجميع ومعالجة إحصائيات الأزرار (تنسيق نهائي ومضغوط)
async function processAndFormatTopButtons(interval) {
    ...
    try {
        ...
        const formattedRows = rows.map((row, index) => {
            ...
            // ✨ التعديل هنا: استخدام تنسيق الكود ودمج الترقيم مع الاسم
            return `${index + 1}\\. \`${escapeMarkdownV2(row.text)}\`\n   \\- 🖱️ الضغطات: \`${row.clicks_count}\`${userText}`;
        }).join('\n\n'); // ✨ التعديل هنا: تقليل المسافة لتكون سطراً واحداً فارغاً
